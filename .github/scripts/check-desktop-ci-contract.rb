...
module DesktopCiContract
  ...
  def check(directory = File.expand_path("../workflows", __dir__))
    ...
    assert(test_jobs.fetch("desktop-tests").fetch("uses") == "./.github/workflows/desktop-tests.yml", "PR workflow must call the desktop test gate")
    %w[desktop-mac-app desktop-windows-app].each do |name|
      package_job = test_jobs.fetch(name)
      assert(needs(package_job).include?("desktop-tests"), "PR #{name} must depend on desktop tests")
    ...
    assert(release_jobs.fetch("desktop-tests").fetch("uses") == "./.github/workflows/desktop-tests.yml", "release workflow must call the desktop test gate")
    assert(needs(release_jobs.fetch("build")).include?("desktop-tests"), "release CLI builds must depend on desktop tests")
    %w[desktop-mac-app desktop-windows-app].each do |name|
      package_job = release_jobs.fetch(name)
      assert(needs(package_job).include?("desktop-tests"), "release #{name} must depend on desktop tests")
    ...
    assert(needs(publication).include?("build"), "publication must remain downstream of qualified builds")
    assert(publication.fetch("if").include?("needs.build.result == 'success'"), "publication must explicitly require its desktop-qualified build dependency")
    true
  end
end
