...
assert(release_jobs.key?("desktop-tests"), "stable release must contain desktop and pinned Pi qualification")
assert(release_jobs.fetch("build").fetch("needs").sort == ["desktop-tests", "test"], "stable build must depend on Go and desktop qualification")
