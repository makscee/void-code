...
- `piModelRetirements` in `cmd/vc/pi_settings.go` drives one shared pre-resolution reconciler for CLI, packaged desktop, and direct bootstrap. It loads and migrates global settings plus an existing project `.pi/settings.json`; an exact retired project pair takes precedence over an exact retired global pair. Foreign providers, provider-less model strings, missing project files, and unrelated settings are preserved. The existing locked atomic settings writer supplies malformed/read-only diagnostics.
...
Both terminal VC and packaged desktop install the same extension and call the shared global/project reconciler after access admission. When Pi loads the managed extension directly, `pi-bootstrap` runs that reconciler and returns an exact provider-bound startup selection. The extension invokes `setModel` with that registered `void-codex` successor before the first turn, so catalog order and other authenticated providers cannot intercept it. Reconciliation warnings are carried in bootstrap JSON and re-emitted on Pi's production stderr rather than disappearing in captured child stderr. Desktop resumes pass the discovered session to `vc desktop-session`; Pi's `session_start` hook performs the session migration. This also covers a session selected later through Pi's native resume UI.
