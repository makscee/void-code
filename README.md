...
accent-coloured row, including parallel calls; completed calls collapse into one
short history line each. Full calls/results remain available through `Ctrl+O`,
and Pi's native reasoning can be toggled with `Ctrl+T`.
...
### Where the Windows bundle's vc comes from

It is compiled from the tree being packaged, by
`desktop/scripts/assemble-windows-resources.mjs`, exactly as the macOS bundle's
is. There is no downloadable vc pin and nothing to qualify: the manifest records
the sha256 of the bytes that were staged and the commit `git rev-parse HEAD`
answered with, so the binary in the installer can always be traced to a revision
you can check out.

It used to be a file downloaded from a pinned past release, and that is what
shipped `vc v0.2.47` inside the v0.2.48 installer — the desktop spawns
`vc login --json`, a flag that release predates, so every Windows sign-in died
on `unknown flag: --json`. Only the Node distribution stays pinned and
downloaded, because nobody here builds it.
