...
if (identity.productName !== 'Void Code' || identity.displayName !== 'Void Code' || identity.version !== '0.1.3-beta.5' || identity.appId !== 'works.voidcode.desktop') throw new Error(`packaged identity mismatch: ${JSON.stringify(identity)}`);
...
const expectedApi = ['auth', 'chooseFolder', 'input', 'lifecycleStatus', 'locale', 'onExit', 'onOutput', 'onStatus', 'openLink', 'resize', 'start', 'status', 'stop', 'support', 'teardown', 'update', 'workspace'];
