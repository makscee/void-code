...
import { assertPackagedIdentity } from './packaged-check-lib.mjs';
...
// The version is checked against the manifest the bundle carries, not against a
// literal: a build that finally reported its true version would otherwise turn
// this smoke red for being correct.
const manifest = JSON.parse(await readFile(path.join(resources, 'manifest.json'), 'utf8'));
assertPackagedIdentity(identity, manifest);
...
const expectedApi = ['appVersion', 'auth', 'chooseFolder', 'input', 'lifecycleStatus', 'onExit', 'onOutput', 'onStatus', 'openLink', 'resize', 'start', 'status', 'stop', 'support', 'teardown', 'workspace'];
