...
it.skipIf(!gated)('R8: actual consumer selection reaches isolated OS clipboard twice, independently read back', () => {
  ...
  try {
    ...
    const output = execFileSync(process.execPath, [entry!, '--offline', '--no-extensions', '--no-skills', '--no-prompt-templates', '--no-themes', '--no-context-files', '-e', path.join(work, 'probe.ts'), '--list-models'], {
      cwd: work, env, encoding: 'utf8', timeout: 25000, maxBuffer: 2 * 1024 * 1024,
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    expect(output).toContain('ASTRA_NATIVE_SELECTION_READBACK_OK_2');
