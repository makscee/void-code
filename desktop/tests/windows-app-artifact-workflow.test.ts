...
// ---------------------------------------------------------------------------
// Packaging without running is how win32 came to have no verification path at
// all. This job builds the installer and uploads it; nothing in between starts
// it, and the one check that does exercise a bundle silences itself on Windows
// and runs on macOS anyway. So the artifact leaving this job has never been
// executed on the system it is for.
//
// What is pinned here is the presence and the position of a step that RUNS the
// bundled smoke against a win32 bundle. Pinned on the script it calls rather
// than on the step's name: a name is a label anybody can put on anything, and a
// placeholder step called "Verify the bundle" would satisfy a name and check
// nothing.
//
// What it cannot do, and this is the same limit the rest of this file states:
// it fixes the FORM. Whether the smoke passes on a windows-latest runner is
// settled by a runner, and nobody here has watched it try.
// ---------------------------------------------------------------------------
const smokeIndex = steps.findIndex((step) => /check[:-]bundled-pi-smoke/.test(asText(step.run)));
const smokeStep = smokeIndex < 0 ? undefined : steps[smokeIndex];
const smokeText = smokeStep === undefined ? '' : `${asText(smokeStep.run)}\n${Object.values(asMap(smokeStep.env)).map((value) => asText(value)).join('\n')}`;

describe(`${REUSABLE} runs the bundle it packages, rather than only packaging it`, () => {
  it('has a step that executes the bundled Pi smoke, named by the script it calls', () => {
    expect(reason(smokeIndex < 0 ? 'nothing in the Windows job runs the bundled Pi smoke, so the installer is uploaded unexecuted' : 'the bundled Pi smoke runs'))
      .toBe('the bundled Pi smoke runs');
  });

  it('asks that smoke for a win32 bundle, not for whatever the runner happens to be', () => {
    // The runner is windows-latest, so a host-derived target would look right here by accident --
    // and would go on being wrong everywhere else the smoke is called from. Stating the target in
    // the step is what makes the job's intent readable without knowing where it runs.
    expect(reason(smokeIndex < 0 ? 'no bundled Pi smoke step at all'
      : /win32-x64/.test(smokeText) ? 'the step names the win32-x64 target'
        : 'the step leaves the bundle target unstated, so it falls to whatever the runner is'))
      .toBe('the step names the win32-x64 target');
  });

  it('runs it before the installer is uploaded, so a broken bundle stops the artifact', () => {
    // Position, not mere presence: a check after the upload leaves a downloadable installer behind
    // for every failure it catches, which is the shape of failure this whole task exists to remove.
    expect(reason(smokeIndex < 0 ? 'no bundled Pi smoke step at all'
      : uploadIndex < 0 ? 'no actions/upload-artifact step'
        : smokeIndex < uploadIndex ? 'smoke, then upload' : 'uploads before it has run anything'))
      .toBe('smoke, then upload');
  });
});
