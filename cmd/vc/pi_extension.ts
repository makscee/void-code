...
export function parseBootstrap(input: unknown): { ok: false } | { ok: true; bootstrap: Bootstrap & { modelDecision: ModelDecisionConfig } } {
	...
	try {
		...
		if (input.relayUrl !== input.relayUrl.trim() || !["https:", "http:"].includes(url.protocol) || !url.hostname || url.username || url.password || (url.pathname !== "" && url.pathname !== "/") || url.search || url.hash) return { ok: false };
