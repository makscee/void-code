...
import { isManagedVoidSearchAvailable, isOpenAISearchAvailable, searchWithOpenAI } from "./openai-search.ts";
...
export async function search(query: string, options: FullSearchOptions = {}): Promise<AttributedSearchResponse> {
	...
	// A managed relay route is authoritative: surfacing its failure preserves the server-only credential boundary.
	if (provider === "auto" && await isManagedVoidSearchAvailable(options.extensionContext)) {
