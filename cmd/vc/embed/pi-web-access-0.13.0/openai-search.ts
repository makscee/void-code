...
interface OpenAIAuth {
	provider: "void-codex" | "openai-codex" | "openai";
	baseUrl?: string;
	apiKey: string;
	model: string;
				...
				try {
					...
					if (resolved.ok && resolved.apiKey) {
						return {
							...
							...(candidate.provider === "void-codex" ? { baseUrl: model.baseUrl } : {}),
...
export async function isManagedVoidSearchAvailable(ctx?: ExtensionContext): Promise<boolean> {
	return (await resolveOpenAIAuth(ctx))?.provider === "void-codex";
}
...
export async function searchWithOpenAI(
	...
	const useCodexEndpoint = auth.provider === "void-codex" || auth.provider === "openai-codex" || isCodexJwt(auth.apiKey);
	if (useCodexEndpoint) {
	...
	try {
		const responsesUrl = auth.provider === "void-codex"
			? `${auth.baseUrl?.replace(/\/$/, "")}/codex/responses`
